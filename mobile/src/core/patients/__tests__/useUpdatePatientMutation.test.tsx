...
function renderMutation(queryClient: ReturnType<typeof createTestQueryClient>) {
  ...
  return renderHook(() => useUpdatePatientMutation(), { wrapper });
}
...
describe('useUpdatePatientMutation', () => {
  ...
  it('atualiza o cache do detalhe quando a atualização tem sucesso', async () => {
    const updatedPatient = { ...fakePatient, name: 'Maria Nova' };
    mockedUpdatePatient.mockResolvedValue(updatedPatient);
    const queryClient = createTestQueryClient();
    queryClient.setQueryData(['patient', 'patient-1'], fakePatient);

    const { result } = await renderMutation(queryClient);

    result.current.mutate({ id: 'patient-1', fields: { name: 'Maria Nova' } });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(queryClient.getQueryData<Patient>(['patient', 'patient-1'])).toEqual(updatedPatient);
  });

  it('invalida a lista de pacientes quando a atualização tem sucesso', async () => {
    const updatedPatient = { ...fakePatient, name: 'Maria Nova' };
    mockedUpdatePatient.mockResolvedValue(updatedPatient);
    const queryClient = createTestQueryClient();
  ...
  it('propaga o erro para o chamador sem reverter estado otimista (não existe)', async () => {
    mockedUpdatePatient.mockRejectedValue(new Error('update failed'));
    const queryClient = createTestQueryClient();
    queryClient.setQueryData(['patient', 'patient-1'], fakePatient);
